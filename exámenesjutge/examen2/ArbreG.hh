...
public:
  ...
  // sortir d'un �mbit de visibilitat
  {
   ...
   T max_suma_cami() const {
		T maxsuma, suma;
		maxsuma = suma = 0;
		suma = aux_max_suma(primer_node, maxsuma, suma);
		return maxsuma;
   }

   T aux_max_suma(node_arbreGen *m, T maxsuma&, T suma) {
		T maxsuma, suma;
		suma = m->info;
		for (int i = 0; i < m->seg.size(); ++i) {
			suma += aux_max_suma(m->seg[i], maxsuma, suma);
  ...
  //// extensi�: programeu-la en un altre fitxer i traieu el comentari
