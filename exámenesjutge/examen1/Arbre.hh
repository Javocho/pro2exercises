...
/* Pre: p.i. = A, asub es buit */
 /* Post: si A conte x, asub es el subarbre d'A resultat de la cerca;
	...
	void sub_arrel(Arbre& asub, const T& x) {
		asub.copia_node_arbre(aux_sub_arrel(primer_node, x));
}

static node_arbre *aux_sub_arrel(node_arbre *a, const T& x) {
		if (a != NULL) {
			if (a->info == x) return a;
			aux_sub_arrel(a->segE, x);
			aux_sub_arrel(a->segD, x);
		}
		return NULL;
}
...
static node_arbre *aux_sub_arrel(node_arbre *a, const T& x, int profactual, int& mejorprof) {
		...
		if (a != NULL) {
			...
			if (aux == NULL) aux = copia_node_arbre(aux_sub_arrel(a->segD, x, profactual++, mejorprof));
			if (aux != NULL) mejoraux =  copia_node_arbre(aux);
		}
